...
Kotatsu Sync is a browser extension that lets you sync playback between two instances of [animepahe](animepahe.pw) so you can watch together with a friend.
...
A common misconception is that WebRTC is used to stream the video itself. It isn't. The video continues to come directly from [animepahe](animepahe.pw). Only metadata such as playback state, timestamps, and sync events are exchanged between peers.
...
As far as I know there isn't a good solution yet, if you have the time or the solution for that, [it would make for a fun side project](github.com/new) \*smug\*
